<script lang="ts">
  ...
  function loadItemByTeam() {
    if (typeof window === "undefined") return [];
    return window.db.items.loadItemsOfTeam(input.org.value, itemType);
  }
  ...
  function onSave() {
    ...
    resultIds = window.db.items.backItem(item_id, team_id, quantity, date);
  }
